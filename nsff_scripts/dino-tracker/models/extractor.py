    ...
    def _register_hooks(self, **kwargs):
        for block_idx, block in enumerate(self.model.blocks):
            ...
            if block_idx in self.layers_dict[VitExtractor.ATTN_KEY]:
                # DINOv2 ≥ 2024: attn_drop is stored as float, not nn.Module.
                # Fall back to proj_drop (nn.Dropout) which is always a module.
                import torch.nn as _nn
                _attn_hook_target = (
                    block.attn.attn_drop
                    if isinstance(block.attn.attn_drop, _nn.Module)
                    else block.attn.proj_drop
                )
                self.hook_handlers.append(_attn_hook_target.register_forward_hook(self._get_attn_hook()))
