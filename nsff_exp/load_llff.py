...
def _load_data(basedir, start_frame, end_frame, step_frame=1,
               factor=None, width=None, height=None, 
               load_imgs=True, evaluation=False, disp_model='midas'):
    print('factor ', factor)
    ...
    imgfiles = imgfiles[start_frame:end_frame:step_frame]
    ...
    if disp_model == 'aligned': # depth anything + metric depth aligned
        disp_dir = os.path.join(basedir, 'disp_aligned')
        
    elif disp_model == 'depthcrafter':
        disp_file = glob(os.path.join(basedir, 'disp_depthcrafter', '*.npz'))[0]
        # disp = np.load(disp_file)['depth'][:end_frame - start_frame:step_frame, ...]
        disp = np.load(disp_file)['depth'][start_frame:end_frame:step_frame, ...]
        resized_disp_list = []
        for i in range(disp.shape[0]):
            resized_disp_list.append(cv2.resize(disp[i], (imgs.shape[1], imgs.shape[0]), interpolation=cv2.INTER_NEAREST))
        disp = np.stack(resized_disp_list, axis = -1)
        
    elif disp_model == 'depth-anything':
        depth_dir = os.path.join(basedir, 'depth-anything')
        depthfiles = [os.path.join(depth_dir, f) for f in sorted(os.listdir(depth_dir)) if f.endswith('npy')]
        depthfiles = depthfiles[start_frame:end_frame:step_frame]
        
        depth = [cv2.resize(read_DepthAnything_depth(f), (imgs.shape[1], imgs.shape[0]), interpolation=cv2.INTER_NEAREST) for f in depthfiles]
        depth = np.stack(depth, -1)
        
        disp = depth
        # disp = 1./ (depth + 1e-5)
    
        
    else: # origin for midas depth
        disp_dir = os.path.join(basedir, 'disp')
        dispfiles = [os.path.join(disp_dir, f) \
                    for f in sorted(os.listdir(disp_dir)) if f.endswith('npy')]
        dispfiles = dispfiles[start_frame:end_frame:step_frame]
    ...
    maskfiles = maskfiles[start_frame:end_frame:step_frame]
...
def poses_avg(poses):
    ...
    center = poses[:, :3, 3].mean(0)    # Axis 0을 따라 평균 계산, Input pose의 개수 방향으로 평균 때림
...
def read_DepthAnything_depth(depth_fi, h=None, w=None):
    depth = np.load(depth_fi)
    return depth
...
def load_hdr_blender_data(basedir, start_frame, end_frame, step_frame,
...
def load_llff_data(basedir, start_frame, end_frame, step_frame,
                   factor=8, target_idx=10, 
                   recenter=True, bd_factor=.75, 
                   spherify=False, path_zflat=False, 
                   final_height=288,
    ...
    poses = np.concatenate([poses[:, 1:2, :], 
                            -poses[:, 0:1, :], 
                            poses[:, 2:, :]], 1)
    poses = np.moveaxis(poses, -1, 0).astype(np.float32)    # 마지막 축 (-1)을 0으로 이동
    images = np.moveaxis(imgs, -1, 0).astype(np.float32)
    bds = np.moveaxis(bds, -1, 0).astype(np.float32)
    ...
    poses = poses[::step_frame, ...] # As the recenter slice the pose here
    
    if poses.shape[0] != images.shape[0]:
        print( 'Mismatch between imgs {} and poses {} !!!!'.format(images.shape[0], 
                                                            poses.shape[-1]) )
        return
