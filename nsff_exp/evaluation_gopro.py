...
def evaluation():
    ...
    # ── Data Loading ───────────────────────────────────────────────────────────
    # load_nvidia_data returns all 9 camera poses for multi-view GoPro evaluation.
    ...
    # Optionally skip the first N frames (e.g. for partial re-evaluation)
    if args.eval_start_time != 0:
        i_train = i_train[args.eval_start_time:]
    ...
    with torch.no_grad():
        ...
        for img_i in i_train:
            ...
            # Output directories: one sub-folder per time step
            ...
            for camera_i in range(9):
                ...
                if Cam_param is not None:
                    ...
                    mu      = torch.tensor(50, dtype=rgb_hdr.dtype, device=rgb_hdr.device)
                    ...
                    rgb_ldr = Cam_param.RAD2LDR_img(ret['rgb_map_ref'], camera_i).cpu().numpy()
                else:
                    rgb_ldr = ret['rgb_map_ref'].cpu().numpy()
